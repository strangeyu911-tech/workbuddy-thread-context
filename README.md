...
交接稿只是一个文件，**新会话不会自动知道它存在**。三种把它递进去的方式：

| 方式 | 在新会话里说什么 |
|---|---|
| **给路径**（最稳） | `读 D:\…\branches\318118d1-n1-7.md，从 #7 之后接着做` |
| 说人话 | `接着 318118d1 从 #7 之后继续`（装了技能时，模型会自己跑一遍导出） |
| 当附件发 | 直接把 `.md` 拖进输入框 |

- 脚本扫的是 `projects/*/`，**跨工作区照样能找到那条会话**——新会话不必开在同一个工作区。
- **分叉只搬上下文，不回滚代码。** 磁盘上的文件仍是此刻的状态；要推翻某个决定，代码得自己 `git reset --hard`。
- 切在 **user 消息**上时，新会话正好停在一个**待回答的问题**处——最理想的续接点。
- 换个窗口宽度就重新导出，文件名带范围后缀（`-n1-7`），不会互相覆盖。
