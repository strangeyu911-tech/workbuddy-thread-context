...
交接稿只是一个文件，**新会话不会自动知道它存在**——必须由人把它「递」进去。三种方式，按可靠度排：

| 方式 | 在新会话里说什么 | 前提 |
|---|---|---|
| **给路径**（最稳） | `读 D:\…\branches\318118d1-n1-7.md，从 #7 之后接着做` | 无（就是普通 Read） |
| 说人话 | `接着 318118d1 从 #7 之后继续` | 装了本技能（模型会自己跑一遍导出） |
| 当附件发 | 直接把 `.md` 拖进输入框 | 无 |

- **技能装在用户级目录**，任何工作区的新会话都能用；脚本的 `--id` 遍历 `projects/*/`，**跨工作区照样能找到那条会话**——新会话不必开在同一个工作区。
- 🔴 **分叉只搬上下文，不回滚代码。** 磁盘上的文件仍是此刻的状态。若分叉是为了推翻某个决定，代码得自己 `git reset --hard` 或改回去——这与 Codex「在新工作树中创建分支」是两件事。
- 切在 **user 消息**上时，新会话正好停在一个**待回答的问题**处（本次 `--until 7` 就是这样）。这是最理想的续接点：原来的答法被丢掉，可以换个方向重答。
- 交接稿是**快照**，原会话仍在追加。窗口不够就重新导出更大的范围；文件名带范围后缀（`-n1-7` / `-n10-11`），不会互相覆盖。
- 落点 `~/.workbuddy/branches/` **不会自动清理**，且可能含敏感正文——别提交进公开仓库。
...
- **1.1.1** —— 补文档：新增「在新会话里续上（用户视角）」。交接稿不会自己跑进新会话，必须由人递给它（给路径 / 说人话 / 当附件）；并写明**分叉只搬上下文、不回滚代码**，以及「切在 user 消息上 = 停在一个待回答的问题处」。脚本无改动。
